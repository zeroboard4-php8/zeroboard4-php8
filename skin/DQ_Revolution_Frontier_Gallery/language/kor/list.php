...
// 목록 버튼
	if($view_once) $_strList = "-목록보기"; else $_strList = "-새로고침";
	$bt_list = $a_list."<font class=han style=font-weight:bold>".$_strList."</font></a>&nbsp;&nbsp;";

// 게시물정리
	if(!eregi("<Zeroboard",$a_delete_all)) $bt_delete_all = $a_delete_all."<font class=han style='font-weight:bold'>-게시물정리</font></a>&nbsp;&nbsp;";

// 이전페이지, 다음페이지
	if(!eregi("<Zeroboard",$a_1_prev_page)) $bt_1_prev_page = $a_1_prev_page."<font class=han style=font-weight:bold>-이전페이지</font></a>&nbsp;&nbsp;";
	if(!eregi("<Zeroboard",$a_1_next_page)) $bt_1_next_page	= $a_1_next_page."<font class=han style=font-weight:bold>-다음페이지</font></a>&nbsp;&nbsp;";
