...
<tr align=center height=30 onMouseOver=this.style.backgroundColor='131313' onMouseOut=this.style.backgroundColor=''>


  <?=$hide_cart_start?><td><input type=checkbox name=cart value="<?=$data['no']?>"></td><?=$hide_cart_end?>

<td class=en align=center><?=$number?></td>

 <?=$hide_category_start?><td><font color=bca800><?=$category_name?></font></td><?=$hide_category_end?>

  <td align=left style='word-break:break-all;'><?=$subject?><font class=en>

<?php
$comment_num="&nbsp;<font color=d6bf03>, ".$data['total_comment']."<b>";
if($data['total_comment']==0) {
  $comment_num="";
}
echo "$comment_num";?> <?=$insert?><?=$icon?></font></td> 
<td align=center><?=$name?></td>
<td class=num><?=$reg_date?></td>
