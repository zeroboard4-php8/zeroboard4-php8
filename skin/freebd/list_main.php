 ...
 * <?=$loop_number?> : 현재 선택되어 있는 글이라도 번호로 나오게
 ...
 참고: old_head.gif : 원본글이면서 12시간이 넘은 글의 아이콘
       ...
       secret_head.gif : 비밀글일때 나타나는 아이콘
...
<tr height=20 align=center bgcolor='<?=$list_main_bg?>' onmouseover="this.style.backgroundColor='<?=$list_main_over_bg?>'" onmouseout="this.style.backgroundColor=''">

  <td></td>

  <td align="center" style=font-family:Arial;font-size:8pt; style='padding:0 5'><?=$number?></td>

<?=$hide_cart_start?><td><input type=checkbox name=cart value="<?=$data['no']?>"></td><?=$hide_cart_end?>

  <td align=left style='word-break:break-all;'><?=$insert?><?=$icon?><?=$subject?> <font style=font-family:Arial;font-size:7pt;><?=$comment_num?></td>

  <td align='<?=$head_name_align?>'><?=$face_image?> <?=$name?></div></td>

  <td align="center" style=font-family:Arial;font-size:8pt;><?=$reg_date?></td>

  <td align="center" style=font-family:Arial;font-size:8pt; style='padding:0 5'><?=$hit?></td>

  <td></td>

</tr>
<tr>
  <td height=1 bgcolor='<?=$list_main_line?>' colspan=10><img height=1></td>
</tr>
