...
<!-- 목록 부분 시작 -->
	...
	<tr height=28 align=center>
		<td class="ver7"><?=$number?></td>
		<?=$hide_cart_start?>
		<td><input type=checkbox name=cart value="<?=$data['no']?>"></td>
		<?=$hide_cart_end?>
		<?=$hide_category_start?>
		<td align="center"><img height=3><br><?=$category_name?></td>
		<?=$hide_category_end?>
		<td align=left style='word-break:break-all;'><img height=2><br><?=$insert?><?=$icon?><?=$subject?> <span class="comment_number"><?=$comment_num?></span></td> 
		<td style='word-break:break-all;'><img height=3><br><?=$face_image?>&nbsp;<?=$name?>&nbsp;</td>
		<td class="ver7"><?=$reg_date?></td>
		<td class="ver7"><?=$hit?></td>
	</tr>
	<tr>
	  <td colspan="7" background="<?=$dir?>/line_back.gif"><img src="<?=$dir?>/line_back.gif" border="0"></td>
	</tr>