...
<tr height=22 align=center bgcolor='<?=$list_main_bg?>' onmouseover="this.style.backgroundColor='<?=$list_main_over_bg?>'" onmouseout="this.style.backgroundColor=''">

  
<td></td>

  
<td class=thm8 style='padding:0 5'><?=$number?></td>


<?=$hide_cart_start?><td><input type=checkbox name=cart value="<?=$data['no']?>"></td><?=$hide_cart_end?>

  
<td align=left style='word-break:break-all;'><img width=1 height=3><br><?=$insert?><?=$icon?><?=$subject?> <font class=thm6><?=$comment_num?></td>

  
<td align='<?=$head_name_align?>'><img width=1 height=3><br><?=$face_image?> <?=$name?></div></td>

  
<td class=thm8><?=$reg_date?></td>

  
<td class=thm8 style='padding:0 5'><?=$hit?></td>

  
<td class=thm8 style='padding:0 5'><?=$vote?></td>
<td></td>
