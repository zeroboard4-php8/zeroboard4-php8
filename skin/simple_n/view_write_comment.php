...
<td align=right>
  ...
  <tr>
    <td><img src=<?=$dir?>/image/t_name.gif border=0><?=$c_name?>
    <?=$hide_c_password_start?><img src=<?=$dir?>/image/c_pw.gif border=0><input type=password name=password <?=size(8)?> maxlength=20 class=input> <?=$hide_c_password_end?>
	</td>
	<td align=right><input type=image src=<?=$dir?>/image/comment.gif border=0 onfocus=blur() accesskey="s"></td>
  </tr>
    </table>
  <br>
  </td>
